...
const Discipline = () => {
        ...
        onSuccess: () => {
          ...
          toast.success(
            editingId
              ...
              : 'Proceeding logged successfully'
          );
          closeModal();
        },
        onError: (err: any) => {
  ...
  return (
    <AppShell>
      ...
      <Section
        title={`Discipline Log ${isLoading ? '...' : filteredRecords.length} Records`}
      >
        <div className="mb-5 relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
          <input
            placeholder="Search records..."
            className="w-full h-11 pl-10 pr-3 bg-muted/20 border border-border rounded-sm focus:outline-none focus:border-accent"
            value={search}
              ...
              {filteredRecords.length === 0 ? (
                <tr>
                  ...
                  >
                    {isLoading
                      ...
                      : 'No disciplinary records found matching search.'}
                  </td>
                </tr>
              ) : (
                filteredRecords.map((r) => (
                  ...
                  >
                    ...
                    <td>
                      <Badge
                        variant={r.status === 'Closed' ? 'success' : 'warning'}
                      >
                        {r.status}
                      </Badge>
                    </td>
                    ...
                    >
                      <div className="flex justify-end gap-1">
                        <button
                          className="p-1.5 rounded-sm hover:bg-primary/10 text-primary"
                          onClick={() => setSelectedCase(r)}
                        >
          ...
          <div className="bg-card w-full max-w-5xl h-[90vh] rounded-md shadow-elevated border border-border flex flex-col overflow-hidden animate-in zoom-in-95">
            <div className="bg-primary px-6 py-4 flex items-center justify-between text-white">
              <div className="flex items-center gap-3">
                ...
                <h3 className="text-lg font-heading font-bold uppercase italic">
                  Case File: {selectedCase.caseId || selectedCase.reference}
                </h3>
                <Badge
                  variant={
                    selectedCase.status === 'Closed' ? 'success' : 'warning'
                  }
                  className="ml-4"
                >
            ...
            <div className="flex-1 overflow-hidden flex">
              <div className="w-1/3 border-r border-border bg-muted/10 p-6 space-y-6 overflow-y-auto">
                <div className="panel p-4 bg-card shadow-sm border-l-4 border-primary">
                  <label className="label-mil text-primary opacity-60 block mb-1">
                    Subject Personnel
                  </label>
                  ...
                  <div className="text-xs font-mono text-muted-foreground">
                    {selectedCase.employee?.serviceNo}
                  </div>
                </div>
              </div>
              <div className="flex-1 p-8 space-y-8 overflow-y-auto">
                <div className="grid grid-cols-2 gap-6">
                  <div className="p-4 bg-destructive/5 border border-destructive/20 rounded-sm shadow-sm">
                    <label className="label-mil text-destructive block mb-1">
                      Initial Offense
                    </label>
                  ...
                  <div className="p-4 bg-warning/5 border border-warning/20 rounded-sm shadow-sm">
                    <label className="label-mil text-warning block mb-1">
                ...
                <div className="space-y-3">
                  <label className="label-mil text-primary opacity-60 flex items-center gap-2">
                    <FileText className="w-3.5 h-3.5" /> Detailed Narrative &
                    Findings
                  </label>
                  <div className="p-6 bg-card border border-border rounded-sm text-sm leading-relaxed shadow-inner min-h-[150px] text-justify whitespace-pre-wrap">
            ...
            <div className="bg-muted/30 p-5 border-t border-border flex justify-between items-center">
              <div className="flex gap-2">
                <Btn variant="outline" onClick={() => setSelectedCase(null)}>
                  Dismiss View
                </Btn>
              </div>
            </div>
        ...
        <div className="fixed inset-0 z-[200] bg-primary/70 backdrop-blur-md flex items-center justify-center p-8">
          <div className="bg-card w-full max-w-md rounded-md shadow-elevated border border-border overflow-hidden animate-in zoom-in-95">
            ...
            <div className="bg-muted/30 p-5 flex justify-end gap-3">
              <Btn variant="outline" onClick={() => setUnlockModal(null)}>
                Cancel
              </Btn>
              <Btn variant="danger" onClick={handleUnlockVerify}>
                Authorize
              </Btn>
            </div>
          </div>
      ...
      {isAdding && (
        <div className="fixed inset-0 z-[150] bg-primary/60 backdrop-blur-sm flex items-center justify-center p-8">
          <div className="bg-card w-full max-w-3xl rounded-md shadow-elevated border border-border animate-in zoom-in-95">
            <div className="bg-destructive px-6 py-4 flex items-center justify-between text-white">
              <h3 className="font-heading font-bold uppercase tracking-wider">
                {editingId ? 'Edit' : 'Log'} Proceeding
              </h3>
            ...
            <div className="p-8 space-y-4 max-h-[80vh] overflow-y-auto">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <Field label="Action">
                  ...
                  >
                    ...
                    <option>Explanation Call</option>
              ...
              <div className="grid grid-cols-2 gap-4">
                <Field label="Category/Type">
                  <Input
                    ...
                    placeholder="e.g. Regular Proceeding"
                  />
                </Field>
              ...
              <Field label="Findings/Narrative">
                <textarea
                  className="w-full p-3 bg-card border border-border rounded-sm text-sm min-h-[100px] outline-none focus:border-accent"
                  value={form.details}
                  onChange={(e) =>
